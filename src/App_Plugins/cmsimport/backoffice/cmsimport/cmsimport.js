...
angular.module("umbraco")
    .controller("cmsimport.wizardController",
        function ($scope, $rootScope, $routeParams, $q, notificationsService, editorService, cmsimportBackofficeResources, cmsimportHelper) {
            ...
            $rootScope.$on("cmsimportElement.changed", function (event, data) {
                $scope.updateModel();
            });
...
angular.module("umbraco").controller("cmsimport.elementPickerDialogController", function ($scope, localizationService) {

    $scope.initialize = function () {
        localizationService.localize("cmsimportElementField_elementPickerDialogTitle").then(function (value) {
            $scope.model.pageName = value;
        });
    };

    $scope.hasNoElements = function () {
        return $scope.model.elementConfig.elementDefinitionConfig.length === 0
    };
    ...
    $scope.select = function (selectedElement) {
        $scope.model.submit(selectedElement);
    };
...
angular.module("umbraco")
    .controller("cmsimport.elementEditorController",
        function ($scope, $rootScope, editorService, cmsimportBackofficeResources, cmsimportHelper) {

            $scope.addElement = function (elementCollection) {
                var options = {
                    localizationKey: "cmsimportScheduledTaskLog_confirmMessage",
                    view: "/App_Plugins/cmsimport/dialogs/elementpicker.html",
                    size: "small",
                    elementConfig: $scope.model.config,
                    submit: function (selectedElement) {
                        editorService.close();
                        $scope.appendField(selectedElement.ContentElementTypeKey, elementCollection)
                    },
                    close: function () {
                        editorService.close();
                    }
            ...
            $scope.appendField = function (elementKey, elementCollection) {
                cmsimportBackofficeResources.appendElement(elementKey).then(function (res) {
                    elementCollection.push(res.data);
                    $rootScope.$broadcast("cmsimportElement.changed", { alias: 'action', value: 'added'});
            ...
            $scope.gePropertyInfo = function (item) {
                return item[0].propertyInfo;
            };

            $scope.removeField = function ($index, elementCollection) {
                elementCollection.splice($index, 1);
                $rootScope.$broadcast("cmsimportElement.changed", { alias: 'action', value: 'removed' });
            };
...
angular.module("umbraco.resources")
    .factory("cmsimportBackofficeResources", function ($http) {
        return {
            ...
            appendElement: function (elementKey) {
                return $http.get("backoffice/cmsimport/cmsimportelementdefinitionapi/getdefinition",
                    { params: { elementDefinitionKey: elementKey } });
            },
        };
    });
